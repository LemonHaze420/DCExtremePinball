...
#define	MAX_SIMULTANEOUS_EFFECT_SAMPLES		4
#define	MAX_SIMULTANEOUS_MUSIC_SAMPLES		4
...
public:
	...
	// ----
	// Initialise the sound system - get hold of the sound device.
	...
	// ----
	// Set master volume - not able to set sample / mod volumes yet.
	...
	// ----
	// Play a sample - this method is useful if the code
	// you are plugging into directly chooses the channel 
	// it wishes to play in.
	...
	// ----
	// Play a sample - this method is useful if you simply
	// want to play a sample and don't care where it plays.
	// You can specify that it is the only instance of the
	// sample playing.
	bool	PlaySample(	CSample*	_Sound,
						int			_Frequency		= 100,
						bool		_Unique			= true,
	...
	// Note:	This does not stop mod playback!
	bool	StopAllSamples();
	...
	bool		IsMusicPlaying()	{return ModPlaying;}
	CString*	GetModName();
	CString*	GetModFileName();
...
protected:
	...
	PlayDataStruct			PlayData[MAX_SIMULTANEOUS_EFFECT_SAMPLES];
