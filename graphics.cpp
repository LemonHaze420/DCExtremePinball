...
//	else
...
#ifndef GRAPHICS_MULTIPLE_BUFFERS
...
#endif
	{
		...
		BackBuffer		= new DWORD[(ScreenMemSize >> 2) + 1];
		PrimaryBuffer	= new DWORD[(ScreenMemSize >> 2) + 1];

#ifdef GRAPHICS_MULTIPLE_BUFFERS
		ScreenOne		= new DWORD[(ScreenMemSize >> 2) + 1];
		ScreenTwo		= new DWORD[(ScreenMemSize >> 2) + 1];
#endif
...
void	CGraphics::SwapScreens()
{
	...
	if (GraphicsMemAddress)
	{
		...
		if (true)//PreferencesRef->DoubleBufferingEnabled)
		{
			if (!BackBuffer)
			{
...
bool	CGraphics::DrawScaledSprite(	int				_XPos, 
										int				_YPos,
										CGraphicsData*	_GraphicsData, 
										RECT&			_Region,
										int				_Flip,
										float			_scaleSize)
{
	...
	float   scaleInc = 1 / _scaleSize;
	float	counterX = scaleInc;
	float	counterY = scaleInc;
		...
		)
	{
		...
		else
		{
			ReadOffsetX	= _Region.left;
			ReadOffsetY	= _Region.top;
			ReadDirectionX = counterX;
			ReadDirectionY = counterY;

			counterX -= ReadDirectionX;
			counterY -= ReadDirectionY;
		}
...
/****************************************/
					...
					)
				{
					...
					int UpperX, UpperY;
					UpperX = min(_XPos + ((float)_Region.right * (float)_scaleSize) - _Region.left, (int)DispProperties.cxWidth);
					UpperY = min(_YPos + ((float)_Region.bottom * (float)_scaleSize) - _Region.top, (int)DispProperties.cyHeight);
